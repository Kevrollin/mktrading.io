...
import { settleAllDueTrades } from "@/lib/trading/state-machine";

/**
 * Not wired to anything yet — no cron/scheduler exists in this
 * deployment. Exists so that once one does (Vercel Cron, Supabase
 * pg_cron, a scheduled Edge Function, ...), closing the "abandoned
 * session" settlement gap (lib/trading/state-machine.ts) is a one-line
...
export async function POST(request: Request) {
  ...
  const settledCount = await settleAllDueTrades();
