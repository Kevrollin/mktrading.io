...
import { markCompleted, WithdrawalError } from "@/lib/withdrawals/state-machine";
import { markCompletedSchema } from "@/lib/validation/wallet";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const withdrawal = await markCompleted({
      withdrawalId: id,
